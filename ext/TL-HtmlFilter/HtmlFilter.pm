...
our $XS_LOADERROR;
TL::HtmlFilter->my_bootstrap($TL::VERSION);
...
sub _new {
    ...
    $this->[CONTEXT]        = TL::HtmlFilter::Context->_new;
    $this->[HTML]           = undef; # 文字列
    $this->[OUTPUT]         = []; # TL::HtmlFilter::{Element,Text,Comment}
...
sub _new {
    ...
    $this->[ADDED] = [];
    $this->[DELETED] = undef;
    $this->[CURRENT] = undef; # TL::HtmlFilter::{Element,Comment,Text}
...
sub newElement {
    ...
    TL::HtmlFilter::Element->_new($name);
}
...
sub newComment {
    ...
    TL::HtmlFilter::Comment->_new($str);
}
...
sub add {
    ...
    if (not defined $elem) {
		...
		if ($pkg !~ m/^TL::HtmlFilter::(?:Element|Text|Comment)$/) {
...
package TL::HtmlFilter::ElementBase;
sub isElement {
    my $this = shift;
    (ref $this) eq 'TL::HtmlFilter::Element';
}

sub isText {
    my $this = shift;
    (ref $this) eq 'TL::HtmlFilter::Text';
}

sub isComment {
    my $this = shift;
    (ref $this) eq 'TL::HtmlFilter::Comment';
}

package TL::HtmlFilter::Element;
use constant {
	# 注意: ここを変更した時は XS 側も修正する事。
	NAME   => 0,
	ATTRS  => 1,
...
our @ISA = qw(TL::HtmlFilter::ElementBase);
...
package TL::HtmlFilter::Comment;
our @ISA = qw(TL::HtmlFilter::Text);
use constant {
	STR => TL::HtmlFilter::Text::STR(),
};
...
次の要素/テキスト/コメントを取り出す。
戻り値は二つで、最初の項目は L</"TL::HtmlFilter::Context"> 、
次の項目は L</"TL::HtmlFilter::ElementBase"> のオブジェクトである。

=back


=head3 TL::HtmlFilter::Context
...
指定された要素名を持つ L</"TL::HtmlFilter::Element"> を作成して返す。

=item newText

  $context->newText($str)

指定された内容を持つ L</"TL::HtmlFilter::Text"> を作成して返す。

=item newComment

  $context->newComment($str)

指定された内容を持つ L</"TL::HtmlFilter::Comment"> を作成して返す。
...
引数は文字列または L</"TL::HtmlFilter::ElementBase"> でなければならない。
...
=head3 TL::HtmlFilter::ElementBase

このクラスは以下のクラスの親クラスである。

=over 4

=item L</"TL::HtmlFilter::Element">

=item L</"TL::HtmlFilter::Text">

=item L</"TL::HtmlFilter::Comment">
...
L</"TL::HtmlFilter::Element"> のインスタンスであれば1を返す。

=item isText

L</"TL::HtmlFilter::Text"> のインスタンスであれば1を返す。

=item isComment

L</"TL::HtmlFilter::Comment"> のインスタンスであれば1を返す。

=back


=head3 TL::HtmlFilter::Element

...
L<セッション|TL::TinySession> は L<クッキー|TL::Cookie> から読む。(デフォルト)

=item L<TL::InputFilter::MobileHTML>

QUERY_STRING / stdinからリクエストを読む。
L<セッション|TL::TinySession> はFormから読む。文字コード変換には常にUniJPを使う。

=item L<TL::InputFilter::SEO>

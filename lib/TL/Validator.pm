...
# -----------------------------------------------------------------------------
package TL::Validator;
use strict;
use warnings;
use TL;

use TL::Validator::FilterFactory;
...
sub addFilter {
	...
	while ( my ( $key, $value ) = each %$filters ) {
		...
		$TL->log(
			'TL::Validator' => qq/addFilter { $key } : / . join(
				', ',
				map {
					    $_->{filter}
...
sub check {
	...
	foreach my $key ( keys %{ $this->{_filters} } ) {
		foreach my $filter ( @{ $this->{_filters}->{$key} } ) {
			my $e =
			  TL::Validator::FilterFactory::getFilter(
				$filter->{filter} )
			  ->doFilter( [ $form->getValues($key) ], $filter->{args} );
			if ($e) {
				$TL->log(
					'TL::Validator' => qq/error { $key => ['@{[

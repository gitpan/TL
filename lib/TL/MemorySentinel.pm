...
sub __postRequest {
	...
	if ($remaining < 0) {
		die $TL->newError(
			'memory-leak' =>
			  "TL::MemorySentinel detected a possible memory leak.\n\n".
				$mem_usage,
			"Memory Leak");
	}
}
...
このモジュールは、最初に L<TL/"Main関数"> を抜けた時の仮想メモリサイズを
保存する。そして次回以降に L<TL/"Main関数"> を抜けた際、最初の時点に対する仮想

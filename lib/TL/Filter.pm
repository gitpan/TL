...
L<< $TL->print|TL/"print" >> 、 L<< $template->flush|TL::Template/"flush" >>
で出力されるデータを加工するクラス。

L<< $TL->print|TL/"print" >> 、 L<< $template->flush|TL::Template/"flush" >>
によるコードからの出力は、 L<< $TL->setContentFilter|TL/"setContentFilter" >>
...
=item L<TL::Filter::HTML> - PC向けHTML出力 (デフォルト)

=item L<TL::Filter::MobileHTML> - 携帯電話向けHTML出力

=item L<TL::Filter::CSV> - CSV出力

=item L<TL::Filter::TEXT> - TEXT出力

=item L<TL::Filter::Binary> - バイナリ出力

=item L<TL::Filter::SEO> - SEO出力フィルタ

=item L<TL::Filter::Redirect> - リダイレクト

L<< $TL->location|TL/"location" >> で使用されるフィルタ。
L<< $TL->location|TL/"location" >> を使用した際は、出力フィルタが L<TL::Filter::Redirect> に切り替わる。

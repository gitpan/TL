...
sub _watch {
  ...
  if(ref($data) eq 'SCALAR')
      ...
      if(tied($$data))
	{
      ...
      tie $$data, 'TL::Debug::Watch::Scalar', $data, $name, $level;
    }
      ...
      if(tied(@$data))
	{
      ...
      tie @$data, 'TL::Debug::Watch::Array', $data, $name, $level;
    }
      ...
      if(tied(%$data))
	{
      ...
      tie %$data, 'TL::Debug::Watch::Hash', $data, $name, $level;
    }
  else
    {
...
sub TIESCALAR {
  ...
  use Data::Dumper;
  $TL->log(__PACKAGE__,
	   TL::Debug::Watch::_calledLocation . ' ' .
...
sub STORE {
  ...
  if($_[0]->[2]) {
    TL::Debug::Watch::_watch(0, $_[0]->[1] . '.$', $_[1], $_[0]->[2] - 1);
  }
  $_[0]->[0] = $_[1];
}
...
sub STORE {
  ...
  if($_[0]->[2]) {
    TL::Debug::Watch::_watch(0, $_[0]->[1] . ".[]", $_[2], $_[0]->[2] - 1);
  }
  $_[0]->[0][$_[1]] = $_[2];
}
...
sub PUSH {
  ...
  if($this->[2]) {
    foreach my $data (@_) {
      TL::Debug::Watch::_watch(0, $this->[1] . ".[]", $data, $this->[2] - 1);
    }
  }
  push(@{$this->[0]}, @_);
}
...
sub STORE {
  ...
  if($_[0]->[2]) {
    TL::Debug::Watch::_watch(0, $_[0]->[1] . ".{$_[1]}", $_[2], $_[0]->[2] - 1);
  }
  $_[0]->[0]{$_[1]} = $_[2];
}

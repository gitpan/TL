...
sub getAllHolidays {
	...
	my $table = $TL::DateTime::JPHoliday::HOLIDAY{sprintf '%04d', $this->getYear};
	$table ? { %$table } : {};		
}
...
sub spanSecond {
	...
	if(!ref($other) || !isa($other, ref($this))) {
		$other = $this->clone->set($other);
	}

	my $span = ($other->{jd} - $this->{jd});
	my $day = int($span);
	my $f = $span - $day;
	my $hour = int($f * 24);
	my $min = int(($f * 24 - $hour) * 60);
	my $sec = int(($f * 24 * 60 - $hour * 60 - $min) * 60);
...
sub spanMonth {
	...
	my $greg1 = $this->__getGregorian();
	my $greg2 = $other->__getGregorian();

	($greg2->{year} - $greg1->{year}) * 12 + ($greg2->{mon} - $greg1->{mon});
}
...
sub addSecond {
	...
	$this->clone->setJulianDay($this->{jd} + $sec / 86400);
}
...
sub addMinute {
	...
	$this->clone->setJulianDay($this->{jd} + $min / 1440);
}
...
sub addHour {
	...
	$this->clone->setJulianDay($this->{jd} + $hour / 24);
}
...
sub addDay {
	...
	$this->clone->setJulianDay($this->{jd} + $day);
}
...
sub addMonth {
	...
	$this->clone->setJulianDay($this->__getJulian($greg));
}
...
sub strFormat {
	...
	my $ampm = sub {
		...
		($noon->spanSecond($this) >= 0) ? $_[1] : $_[0];
	};
...
sub __getJulian {
	...
	my $t  = int($greg->{year} / 100);
	my $jd = int($greg->{year} * 365.25) - $t + int($t / 4);
	$jd += int(30.6001 * ($greg->{mon} + 1));
	$jd += $greg->{day};
	$jd += $greg->{hour} / 24;
	$jd += $greg->{min} / 1440;
	$jd += $greg->{sec} / 86400;
...
sub __getGregorian {
	...
	$greg->{hour} = int($f * 24);
	$greg->{min} = int(($f * 24 - $greg->{hour}) * 60);
	$greg->{sec} = int(($f * 24 * 60 - $greg->{hour} * 60 - $greg->{min}) * 60 + 0.5);
...
引数との秒数の差を計算し、結果を整数で返す。引数が DateTime オブジェク
トだった場合はそのオブジェクトと比較し、それ以外の場合は引数をそのまま 
$TL->newDateTime に渡して生成したオブジェクトと比較する。

返される値は (引数) - (this) であり、引数が過去ならば結果は負になる。
...
$sec 秒後の時刻を表す DateTime オブジェクトを新たに生成して返す。
...
次の日を表す DateTime オブジェクトを返す。

=item C<< prevDay >>

前の日を表す DateTime オブジェクトを返す。

=item C<< firstDay >>

その月の最初の日を表す DateTime オブジェクトを返す。

=item C<< lastDay >>

その月の最後の日を表す DateTime オブジェクトを返す。

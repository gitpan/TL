	...
	if($this->{enabled}) {
		...
		if($this->{warn_logging} or $this->{warn_popup}) {
			$SIG{__WARN__} = sub {
				...
				isa($msg, 'TL::Error') or $msg = $TL->newError(warn => $msg);
...
sub _templateLog {
	...
	# nodeはTL::Templateオブジェクトでなくノードパスとする。(/foo/bar形式)
...
sub _formLog {
	...
	# スタックを辿り、最初に現れたTL::以外のパッケージが作ったフレームを見て、
	# ファイル名と行番号を得る。
	my $loc = '';
	for(my $i = 0;; $i++) {
...
このクラスは C<use TL> 時に自動的に読み込まれる．

L<ini|TL::Ini> で L</"enable_debug"> を0以外にセットすると、
以後デバッグ機能が有効になる。
L<ini|TL::Ini> グループ名は "Debug" でなければならない。
...
L<< $TL->log|TL/"log" >> されたログをポップアップ表示するように出力を加工するか否か。省略可能。
...
使用した L<テンプレート|TL::Template> ファイル名と、展開の内容をログに残すか否か。省略可能。
...
使用した L<テンプレート|TL::Template>ァイル名と、展開の内容を、別ウィンドウでポップアップするように出力を加工するか否か省略可能。
...
L<DB|TL::DB> へのクエリをログに残すか否か。省略可能。
...
L<DB|TL::DB> へのクエリログをポップアップ表示するように出力を加工するか否か。省略可能。
...
L<DB|TL::DB> へのクエリをログに残す際、応答内容をどの程度残すかを設定する。省略可能。
...
L<DB|TL::DB> へのクエリをログに残す際、実行計画をどこまで残すかを設定する。省略可能。
...
L<< $TL->location|TL/"location" >> でリダイレクトする際に、Locationヘッダの代わりに、HTML画面を表示する。

...
sub read {
	...
	my $fh = $TL->_gensym;
	if(!open($fh, "$filename")) {
		die "TL::Ini#read, can't open file for read. [$filename] ($!)\n";
	}
	...
	while(<$fh>) {
		...
		if(m/^\[(.+)\]$/) {
			...
			if(defined($group) && defined($key) && defined ($value)) {
				...
				die "TL::Ini#read, ini data format error. line [$.]\n";
			}
		}
	}
	close($fh);
...
sub write {
	...
	my $fh = $TL->_gensym;
	if(!open($fh, ">$filename")) {
		die "TL::Ini#write, can't open file for write. [$filename] ($!)\n";
	}
...
sub existsGroup {
	...
	if(!defined($group)) {
		die "TL::Ini#existsGroup, ARG[1]: got undef.\n";
	} elsif(ref($group)) {
		die "TL::Ini#existsGroup, ARG[1]: got Ref.[$group]\n";
	}
...
sub existsKey {
	...
	if(!defined($group)) {
		die "TL::Ini#existsKey, ARG[1]: got undef.\n";
	} elsif(ref($group)) {
		die "TL::Ini#existsKey, ARG[1]: got Ref.[$group]\n";
	}
	if(!defined($key)) {
		die "TL::Ini#existsKey, ARG[2]: got undef.\n";
	} elsif(ref($key)) {
		die "TL::Ini#existsKey, ARG[2]: got Ref.[$key]\n";
	}
...
sub getKeys {
	...
	if(!defined($group)) {
		die "TL::Ini#getKeys, ARG[1]: got undef.\n";
	} elsif(ref($group)) {
		die "TL::Ini#getKeys, ARG[1]: got Ref.[$group]\n";
	}
...
sub get {
	...
	if(!defined($group)) {
		die "TL::Ini#get, ARG[1]: got undef.\n";
	} elsif(ref($group)) {
		die "TL::Ini#get, ARG[1]: got Ref.[$group]\n";
	}
	if(!defined($key)) {
		die "TL::Ini#get, ARG[2]: got undef.\n";
	} elsif(ref($key)) {
		die "TL::Ini#get, ARG[2]: got Ref.[$key]\n";
	}
...
sub set {
	...
	if(exists($this->{const})) {
		die "TL::Ini#set, This instance is const object.\n";
	}

	if(!defined($group)) {
		die "TL::Ini#set, ARG[1]: got undef.\n";
	} elsif(ref($group)) {
		die "TL::Ini#set, ARG[1]: got Ref.[$group]\n";
	}
	if(!defined($key)) {
		die "TL::Ini#set, ARG[2]: got undef.\n";
	} elsif(ref($key)) {
		die "TL::Ini#set, ARG[2]: got Ref.[$key]\n";
	}
	if(!defined($value)) {
		die "TL::Ini#set, ARG[3]: got undef.\n";
	} elsif(ref($value)) {
		die "TL::Ini#set, ARG[3]: got Ref.[$value]\n";
	}
	if($group =~ m/[\x00-\x1f]/) {
		die "TL::Ini#set, ARG[1]: contains control code.\n";
	}
	if($key =~ m/[\x00-\x1f]/) {
		die "TL::Ini#set, ARG[2]: contains control code.\n";
	}
	if($value =~ m/[\x00-\x1f]/) {
		die "TL::Ini#set, ARG[3]: contains control code.\n";
	}
	if($group =~ m/^\s+/ or $group =~ m/\s+$/) {
		die "TL::Ini#set, ARG[1]: space will be delete.\n";
	}
	if($key =~ m/^\s+/ or $key =~ m/\s+$/) {
		die "TL::Ini#set, ARG[2]: space will be delete.\n";
	}
	if($value =~ m/^\s+/ or $value =~ m/\s+$/) {
		die "TL::Ini#set, ARG[3]: space will be delete.\n";
	}
...
sub delete {
	...
	if(exists($this->{const})) {
		die "TL::Ini#delete, This instance is const object.\n";
	}

	if(!defined($group)) {
		die "TL::Ini#delete, ARG[1]: got undef.\n";
	} elsif(ref($group)) {
		die "TL::Ini#delete, ARG[1]: got Ref.[$group]\n";
	}
	if(!defined($key)) {
		die "TL::Ini#delete, ARG[2]: got undef.\n";
	} elsif(ref($key)) {
		die "TL::Ini#delete, ARG[2]: got Ref.[$key]\n";
	}
...
sub _getrawgroupname {
	...
	my @group;
	foreach my $spec (@TL::specialization, '') {

...
sub decodeCgi {
	...
	if(defined(&TL::Session::_getInstance)) {
		...
		foreach my $group (TL::Session->_getInstanceGroups) {
			TL::Session->_getInstance($group)->_getSessionDataFromCookies;
		}
	}
...
ルハンドルが L<Form|TL::Form> に格納される。詳しくは L<TL> のIniパラメー
タを参照。

=back

また、 L<TL::Session> が有効になっている場合は、セッションデータを
L<クッキー|TL::Cookie> から読み出す。

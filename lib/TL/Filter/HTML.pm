...
# 注意:
# * TL::Filter::HTMLはバッファリングを行わない為、出力されるhtmlの
...
sub _new {
	...
	if($this->{option}{type} ne 'html' && $this->{option}{type} ne 'xhtml') {
		die "TL#setContentFilter, option [type] for [TL::Filter::HTML] ".
...
sub _make_header {
	...
	if(defined(&TL::Session::_getInstance)){
		# TL::Sessionが有効になっているので、データが有れば、それをクッキーに加える。
		foreach my $group (TL::Session->_getInstanceGroups) {
			TL::Session->_getInstance($group)->_setSessionDataToCookies;
		}
	}
	...
	return {
		'Set-Cookie' => [
			TL::Cookie->_makeSetCookies,
			TL::RawCookie->_makeSetCookies,
		],
	};
}
...
=head1 SYNOPSIS
  ...
  $TL->setContentFilter('TL::Filter::HTML', charset => 'UTF-8');
...
=item 漢字コード変換（デフォルトShift_JIS、Encode優先）
...
=item contenttype
  ...
  $TL->setContentFilter('TL::Filter::HTML', contenttype => 'text/html; charset=sjis');
...
=item type
  ...
  $TL->setContentFilter('TL::Filter::HTML', type => 'xhtml');

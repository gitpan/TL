...
# 注意:
# * このフィルタはTL::Filter::HTMLやTL::Filter::MobileHTMLよりも
#   後に実行されるように設定しなければならない。
# * TL::Filter::HTML, TL::Filter::MobileHTML がタグの途中で区切って
#   出力を行わない性質に依存している。
#   それ以外のカスタムフィルタと兼用する場合はその点を注意する
#   必要がある。
# * JISコードでの出力時は正常に動作しない。
# * charsetにはTL::Filter::HTMLまたはTL::Filter::MobileHTMLと
#   同じ文字コードを指定しなければならない。
...
sub toLink {
	...
	} elsif(ref($form) ne 'TL::Form') {
		die __PACKAGE__."#toLink, ARG[1] was not instance of TL::Form. [$form].\n";
	}
	if(ref($base)) {
...
=head1 SYNOPSIS
  ...
  $TL->setContentFilter('TL::Filter::HTML');
...
このフィルタは L<TL::Filter::HTML> や L<TL::Filter::MobileHTML>
よりも後に実行されるように設定しなければならない。

また、リンクを書き換えた場合、そのリクエストは L<TL::InputFilter::SEO>
を使用しなければ正常に受け取れない。
...
=item hide_extension
  ...
  $TL->setContentFilter(['TL::Filter::SEO', 1001], hide_extension => 0);

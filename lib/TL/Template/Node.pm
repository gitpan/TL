...
sub _new {
	my $class = shift;
	my $parent = shift; # TL::Template::Node または undef (rootの場合)
...
sub _setTemplate {
	...
	$str =~ s{<!begin:(.+?)>(.*?)<!end:\1>}{
		...
		$this->{node}{$name} = TL::Template::Node->_new(
			$this, $name, $template
		);
		"<!mark:$name>";
	}egs;
...
sub setForm {
	...
	if(!defined($form)) {
		...
		die __PACKAGE__."#setForm, ARG[1] was not instance of TL::Form. [$form].\n";
	}
...
sub addHiddenForm {
	...
	if(!defined($form)) {
		...
		die __PACKAGE__."#addHiddenForm, ARG[1] was not instance of TL::Form.\n";
	}
	if(ref($name)) {

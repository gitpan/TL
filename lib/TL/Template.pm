# -----------------------------------------------------------------------------
# TL::Template - テンプレートを扱う
# -----------------------------------------------------------------------------
package TL::Template;
use TL;
use strict;
use warnings;
require TL::Template::Node;
require File::Spec;
...
sub _new {
	...
	$this->{root} = TL::Template::Node->_new;
...
L<< $TL->escapeTag|TL/"escapeTag" >> を適用後、出力する

=item br

L<< $TL->escapeTag|TL/"escapeTag" >> を適用後、
...
L<< $TL->escapeJs|TL/"escapeJs" >> を適用後、出力する
...
渡された L<TL::Form> オブジェクトを、HTMLテンプレート中のフォームに
展開する。
...
ノードの場合、そのノードの終端までを L<出力|TL/"print"> する。
Templateの場合、全体を出力する。

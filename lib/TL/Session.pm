...
# [出力時]
#   [1: クッキーを使う場合]
#     TL::FilterがSet-Cookie:ヘッダを出力する。
#     この時、iniパラメータ"cookie" / "securecookie"で指定されたグループ名のTL::Cookieが使われる。
#   [2: クエリを使う場合]
#     TL::Filterが$SAVEに加える。
#
# [入力時]
#   [1: クッキーを使う場合]
#     TL::InputFilterがdecodeCgi中に$TL->getCookieし、その中にセッションデータがあり、
#     且つTL::TinySessionが有効になっていれば、$TL->getTinySession->_setSessionDataする。
#   [2: クエリを使う場合]
#     TL::InputFilterがdecodeCgi中にクエリ内にセッションデータを見付けた場合、
#     TL::TinySessionが有効になっているなら、$TL->getTinySession->_setSessionDataする。
...
sub _init {
	...
	if(ref($_[0]) eq 'ARRAY') {
		...
		my $ref = ref($_[0]);
		die "TL::Session#_init, ARG[1]: group name is Wrong-Ref. [$ref]\n";
	}

	foreach my $group (@$groups) {
		$_instance{$group} = TL::Session->__new($group);
...
sub _getInstance {
	# TL#getSessionやTL::Filterによって呼ばれるクラスメソッド。
	my $class = shift;
	my $group = shift;
...
sub _setSessionDataToCookies {
	# クッキーを使用する場合に，TL::Filter より呼び出される．
...
sub _getSessionDataFromCookies {
	# クッキーを使用する場合に，TL::InputFilter より呼び出される．
...
sub _setSessionDataToForm {
	# フォームを使用する場合に，TL::Filter より呼び出される．
...
sub _getSessionDataFromForm {
	# フォームを使用する場合に，TL::InputFilter より呼び出される．
...
セッションキーは、 L<出力フィルタ|TL/"出力フィルタ"> に L<TL::Filter::HTML>
を使用している場合は L<クッキー|TL::Cookie> に、 L<TL::Filter::MobileHTML>
の場合は L<クエリ|TL::Form> に、それぞれ挿入される。

また、 L<入力フィルタ|TL/"入力フィルタ"> に L<TL::InputFilter::HTML>
を使用している場合は L<クッキー|TL::Cookie> から、L<TL::Filter::MobileHTML>
の場合は L<クエリ|TL::Form> から、それぞれ読み取られる。

Sessionは L<DB|TL::DB> を使用してセッションの管理を行う。
...
引数には L<ini|TL::Ini> で設定したグループ名を渡す。省略可能。

このメソッドは、 L<TL#startCgi|TL/"startCgi">
...
指定の時間経過したセッションは無効とする。L<度量衡|TL/"度量衡"> 参照。省略可能。
...
最終更新時刻から指定時間以上経過していたら、DBの更新時刻を更新する。L<度量衡|TL/"度量衡"> 参照。省略可能。
最短で timeout - updateinterval の時間でタイムアウトする可能性がある。
...
=item L<TL::Filter::MobileHTML>

=item L<TL::InputFilter::HTML>

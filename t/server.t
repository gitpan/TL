...
install(
    script => q{
		...
		sub main {
			...
			$TL->setContentFilter('TL::Filter::TEXT');
			$t->flush;
		}
    },
   );
is(rget->content, 'testdata', 'TL::Template (flush)');
...
is(rget->content, 'testtestdata', 'TL::Template (flush)');
...
is(rget->content, '[set]', 'TL::RawCookie (set)');
...
is(rget->content, '111', 'TL::RawCookie (get)');
...
is(rget->content, '[set]', 'TL::Cookie (set)');
...
is(rget->content, 'MONSTER', 'TL::Cookie (get)');
